...
function App() {
  ...
  const {register, handleSubmit, watch, formState: { errors }} = useForm({
    mode: 'onSubmit', // 최초 검증 시점, default(기본값) onSubmit
    reValidateMode: 'onChange', // 재검증 시점, default(기본값) onChange
    criteriaMode: 'all', // 객체(errors)에 첫 에러 하나만 포함하거나 (firstError) 전부 포함(all), default(기본값): firstError
    defaultValues: {
      name: '',
      email: '',
      cellphone: '010'
  ...
  // handleSubmit에서 검증을 통과할 경우 호출될 함수
  const onSubmit = (user) => {
